...
class DashboardManagerView(Toplevel):
    ...
    def create_filter_controls(self, parent_frame):
        ...
        self.filter_button = tb.Button(parent_frame, text="Aplicar Filtros", bootstyle=SUCCESS, command=self.load_report_data)
    ...
    def load_report_data(self):
        """Função principal que busca os dados, atualiza KPIs, gráficos e tabela."""
        conn = self.get_db_connection()
        if not conn: return
    ...
    def update_kpis(self, df):
        ...
        self.kpi_tempo_paradas_label.config(text=self.format_seconds_to_hhmmss(total_paradas_s))

    def update_charts(self, df):
        """Agrega os dados e redesenha todos os gráficos."""
        ...
        self.draw_bar_chart(
            self.graph_canvas['machine_perf'], data=machine_data, x_col='maquina',
        ...
        if not df_time.empty and 'data_ordem' in df_time.columns:
            ...
            self.draw_line_chart(
                self.graph_canvas['prod_time'], data=time_data, x_col='data_ordem',
        ...
        self.draw_pie_chart(
            self.graph_canvas['loss_pie'], data=loss_data, labels_col='Tipo de Perda',
            values_col='Quantidade', title='Composição das Perdas'
        )
        
        op_data = df.groupby('operador')['prod_qtd'].sum().nlargest(10).reset_index()
        self.draw_bar_chart(
            self.graph_canvas['op_perf'], data=op_data, x_col='operador',
            y_cols=['prod_qtd'], labels=['Produção'], title='Top 10 Operadores por Produção',
            colors=['#17a2b8']
        )

    def update_detailed_table(self, df):
        """Popula a tabela de dados detalhados com os resultados da consulta."""
